...
def ChatGPT_single_request_temperature(prompt, temperature=0.5, time_sleep_second=0.1):
    temp_sleep(time_sleep_second)
...
def ChatGPT_request_temperature(prompt, temperature=0.5):
    """
  Given a prompt and a dictionary of GPT parameters, make a request to OpenAI
  server and returns the response.
  ARGS:
  ...
  """
    # temp_sleep()
    try:
        return ChatGPT_single_request_temperature(prompt, temperature, time_sleep_second=0)
    except Exception as e:
        metrics.fail_record(e)
        print("ChatGPT ERROR")
        return "ChatGPT ERROR"
...
def ChatGPT_safe_generate_response_temperature(prompt,
                                       repeat=3,
                                       fail_safe_response="error",
                                       temperature=0.5,
                                       func_validate=None,
    ...
    for i in range(repeat):
        try:
            curr_gpt_response = ChatGPT_request_temperature(prompt, temperature).strip()
            if func_validate(curr_gpt_response, prompt=prompt):
                return func_clean_up(curr_gpt_response, prompt=prompt)
            if verbose:
                print(f"---- repeat count: {i}")
